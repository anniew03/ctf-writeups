...
**Artifacts:**
* chall.py: encryption program provided by challenge authors
...
Upon connecting to the host, the program will generate random values for a, c, and m, ensuring that they satisfy a valid LCG where $m = 2^{512}$, and then print a, c, and m out.

The LCG is seeded with a random 512-bit value, and the LCG is iterated a random number of times.

As the LCG is used to generate numbers, the program checks if each number is prime. Once it has generated 2 prime numbers, it uses these values to produce $n = pq$ and prints $n$
